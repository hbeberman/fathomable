...
tree. `Workspace::exact_head_commit` pins symbolic or detached `HEAD` without
peeling; exact lookup and blob reads ignore Git replacement objects. Intermediate
path entries are verified as directories and tree objects before loading.
The API rejects missing/corrupt objects, non-commit IDs, non-blob or
