...
boundaries and clamps at the list edges. On a selected thread or file-group
header, `h`/Left collapses and `l`/Right expands. `Tab`/`Shift-Tab` provide
direct open-thread traversal.
...
- **File rows, messages, and folded threads are stops.** `j`/`Down` and
  ...
  the hints keep acting on a thread. On a thread or file-group selection,
  `h`/Left collapses and `l`/Right expands.
